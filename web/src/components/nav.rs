...
impl NavBuilder {
    ...
    pub fn build(self) -> Markup {
        html! {
            div class="flex justify-between py-4 px-8" {
                h1 class="my-auto font-bold text-teal-500" {}
                nav class="flex justify-between w-96" {
    ...
    fn link(name: &str, path: &str, active: bool) -> Markup {
        let link = html! {
            a href=(path) class="px-1 text-2xl font-bold text-teal-500 transition-all hover:text-teal-200"
                { (name) }
        };

        match active {
            true => html! {
                div class="border-b-4 border-teal-500" { (link) }
            },
            false => link,
        }
    }
